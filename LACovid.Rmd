...
```{r}
ggplot() +
  geom_sf(aes(geometry = geom, fill = private_jail_flag), data = jails) +
  labs(title = "Private Jails in Louisiana") +
  geom_sf(data = stategeo, fill = NA) +
...
jails <- left_join(jails, residuals, by = c("obs_num" = "obs_num"))
```

```{r residual plot}
plotresid <- ggplot() +
  geom_sf(aes(geometry = geom, fill = .resid), data = jails) +
  labs(title = "Louisiana", 
       subtitle = "Residuals from Logistic Model") +
  geom_sf(data = stategeo, fill = NA) +
  theme_void() + 
  scale_fill_distiller(palette = 'RdBu', guide = "legend", breaks = brks)
...
averages <- averages %>%
  rename(`Percent of Prisoners White` = avgs) %>%
  rename(`Above Average Decrease in Prisoners` = ab_avg)

ggplot() +
  geom_col(aes(x = `Above Average Decrease in Prisoners`, y= `Percent of Prisoners White`), data = averages, fill = c("red", "skyblue")) 
```
...
ggplot () +
  geom_col(aes(x = obs_num, y = County_Profile, fill = ab_avg), data = plotbiv)
```
...
averagespct <- averagespct %>%
  rename(`% Won by Donald Trump 2016` = avgs) %>%
  rename(`Above Average Decrease in Prisoners` = ab_avg)

ggplot() +
  geom_col(aes(x = `Above Average Decrease in Prisoners`, y= `% Won by Donald Trump 2016`), data = averagespct, fill = c("red", "skyblue")) 
```
...
plotrural <- ggplot() +
  geom_bar(aes(x = `Above Average Decrease in Prisoners`), data = numrural2)

plotrural2 <- ggplot() +
  geom_bar(aes(x = `Low Population Counties (< 50000)`), data = numrural)
